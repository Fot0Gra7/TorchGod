...
public class PlayerMovement : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!IsDashing && !IsJumping)
		{
			...
			{
				//AnimHandler.SetBool("IsGrounded", true);
				//AnimHandler.SetBool("IsOnWall", false);
				//AnimHandler.ResetTrigger("Jump");
			...
			//AnimHandler.SetBool("IsOnWall", true);
		...
		if (!IsDashing)
		{
			...
			if (CanJump() && LastPressedJumpTime > 0)
			{
				...
				//AnimHandler.SetTrigger("Jump");

				//AnimHandler.startedJumping = true;
			}
			//WALL JUMP
			else if (CanWallJump() && LastPressedJumpTime > 0 && canWallJump == true)
			{
				IsWallJumping = true;
				IsJumping = false;
				_isJumpCut = false;
